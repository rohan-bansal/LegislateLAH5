...
async def searchRaw(topic, session, state="ALL"):
    # GET from API
    url = "https://api.legiscan.com/?key={}&op=searchRaw&state={}&query={}&year={}".format(KEY, state, "+".join(i for i in topic.split(" ")), 3)
...
async def searchTopic(topic, relevanceCutoff=60, state="ALL"):
...
if __name__ == "__main__":
    ...
    asyncio.run(searchTopic("Tariffs"))
